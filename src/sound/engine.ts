...
/** "mine" is the player at the board; "theirs" is Euclid and recedes. */
...
export interface GameSounds {
  ...
  /** Fading pieces: stones washing off the board, like a small wave receding. */
  wash(delay?: number): void;
}
...
export function createSoundEngine(): SoundEngine {
  ...
  const wash: GameSounds["wash"] = (delay = 0) => {
    const live = ready();
    ...
    source.buffer = noise;
    source.loop = true;
    // Noise falling from a hiss to a low rush reads as water drawing back.
    const filter = ctx.createBiquadFilter();
    filter.type = "lowpass";
    ...
    envelope.gain.linearRampToValueAtTime(0.14, start + 0.09);
...
export const saveSoundPreference = (on: boolean): void =>
  writeStored(STORAGE_KEYS.sound, on);
