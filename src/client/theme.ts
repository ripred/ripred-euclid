...
export const readThemeModeFromElement = (
  ...
  const attrTheme =
    normalizeThemeMode(element.getAttribute("data-theme")) ||
  ...
  try {
    return normalizeThemeMode(
      sourceWindow.getComputedStyle(element).colorScheme,
    );
  } catch {
    return null;
  }
...
export const detectThemeMode = (): ThemeMode => {
  try {
    ...
    // Cross-origin parent documents cannot be inspected; OS preference remains available.
  }
...
export const installThemeModeSync = (
  ...
  try {
    ...
    // Cross-origin parent documents cannot be observed; media-query syncing still works.
  }
