...
class Board {
  ...
  initGame(){ this.m_board=new Array(this.W*this.H).fill(0); this.m_history=[]; this.m_turn=0; this.m_displayed_game_over=false; this.m_last=new Point(-1,-1,-1); this.m_players[0].initGame(); this.m_players[1].initGame(); this.m_targets=[null,null]; }
  ...
  private scoreSquare(p1:Point,p2:Point,p3:Point,p4:Point):number{
    if(this.scoring==='bbox'){
      ...
      return Number.isFinite(minD2)? minD2 : 0;
    }
  }

  analyze(move:Point,potential:Square[]|null){
    let total=0; const x=move.x,y=move.y,clr=this.m_turn+1;
    if(potential) potential.length=0; const other=(clr===1)?2:1;
    const rows=this.createRandomizedRange(this.H), columns=this.createRandomizedRange(this.W);
    for(let index=0;index<this.W*this.H;index++){
      const row=rows[Math.floor(index/this.W)], col=columns[index%this.W];
      const dx=col-x, dy=row-y, x1=x-dy,y1=y+dx,x2=col-dy,y2=row+dx;
      if(x1<0||x1>=this.W||y1<0||y1>=this.H||x2<0||x2>=this.W||y2<0||y2>=this.H||(col===x&&row===y)) continue;
      const v1=this.m_board[y*this.W+x], v2=this.m_board[row*this.W+col], v3=this.m_board[y1*this.W+x1], v4=this.m_board[y2*this.W+x2];
      if(v1===other||v2===other||v3===other||v4===other) continue;
      const remain=(v1===0?1:0)+(v2===0?1:0)+(v3===0?1:0)+(v4===0?1:0);

      const sq=new Square(this.pointAt(x,y),this.pointAt(col,row),this.pointAt(x1,y1),this.pointAt(x2,y2),clr,0,remain);
      if(remain===0){
        const score=this.scoreSquare(sq.p1,sq.p2,sq.p3,sq.p4);
        sq.points=score;
        total+=score;
  ...
  private scoreIfPlacedForColor(pt:Point, color:number){
    const saved=this.m_board[pt.index]; const savedTurn=this.m_turn;
    this.m_board[pt.index]=color; this.m_turn=color-1;
    ...
    for(let row=0;row<this.H;row++){
      for(let col=0;col<this.W;col++){
        ...
        const remain=(v1===0?1:0)+(v2===0?1:0)+(v3===0?1:0)+(v4===0?1:0);
        if(remain===0){
          const score=this.scoreSquare(this.pointAt(x,y), this.pointAt(col,row), this.pointAt(x1,y1), this.pointAt(x2,y2));
          total+=score;
        }
      }
    }
  ...
  private chooseUnifiedMove(defProb:number, offProb:number):Point{
    ...
    for(let i=0;i<this.m_board.length;i++){
      ...
      const pt=this.pointAt(i%this.W, Math.floor(i/this.W));
    ...
    if(playPt){ if(this.shouldMistake(offProb)) return this.randomEmptyPoint(); return playPt; }
    ...
    for(let x=0;x<W;x++){
      for(let row=0; row<H; row++){
        for(let col=0; col<W; col++){
          const dx=col-x, dy=row-y, x1=x-dy,y1=y+dx,x2=col-dy,y2=row+dx;
          if(x1<0||x1>=W||y1<0||y1>=H||x2<0||x2>=W||y2<0||y2>=H||(col===x&&row===y)) continue;
          const idxs=[y*W+x, row*W+col, y1*W+x1, y2*W+x2].sort((a,b)=>a-b).join(',');
          if (seen.has(idxs)) continue;
          seen.add(idxs);
          sum += scoreSquare({x,y},{x:col,y:row},{x:x1,y:y1},{x:x2,y:y2});
        }
      }
    }
  }
...
export const App=(context:Devvit.Context)=>{
  ...
  const sendChat = async () => {
    ...
    if (mode==='ai') {
      ...
      const gid = gameIdRef.current; if(!gid) { setChatOpen(false); return; }
      try{
        await fetch('/api/h2h/chat',{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ gameId: gid, text }) });
        setTimeout(refreshStateOnce, 150);
      }catch{}
      setChatText(''); setChatOpen(false);
    } else {
      setChatOpen(false);
  ...
  },[mode,board,isPlayer1,spectating,winner,finalSide,selectedStyle,chatOpen]);
  ...
  const ChatOverlay = !chatOpen ? null : (
    <div style={{position:'fixed', inset:0, display:'flex', alignItems:'flex-end', justifyContent:'center', padding:'0 12px 18px', zIndex:70, background:'rgba(0,0,0,.18)'}}
         onClick={()=>{ setChatOpen(false); }}>
      <div onClick={(e)=>e.stopPropagation()} style={{display:'flex', gap:8, width:'min(720px, 96vw)', background:'var(--card-bg)', border:`1px solid var(--card-border)`, borderRadius:10, padding:8}}>
        <input
          autoFocus
          value={chatText}
...
const GameScreen:React.FC<{
  ...
  const stackScores = isMobile || bw > vw * 0.82;
  ...
  const addLinesFading=(sqs:Square[], rgbVar:string)=>{ const n=sqs.length, minA=0.14, maxA=0.9;
    for(let idx=0; idx<n; idx++){ const sq=sqs[idx]; const a=n<=1?maxA:(minA+(idx/(n-1))*(maxA-minA));
      const ord=orderByAngle([sq.p1,sq.p2,sq.p3,sq.p4]).map((p:any)=>({x:(p.x+0.5)*cell,y:(p.y+0.5)*cell}));
    ...
    <div className="flex flex-col items-center gap-4 p-4" style={{background:'var(--bg)', height:'100vh', overflow:'hidden'}}>
      ...
      {(p1Name && p2Name) ? (!stackScores ? (
        <div className="w-full flex justify-between gap-4 items-start">
          ...
          <div className="flex flex-col items-center justify-start" style={{minWidth:220, color:'var(--text)'}}>
