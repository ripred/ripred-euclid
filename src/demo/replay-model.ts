...
import { scoreSquareCorners } from "../game/scoring";
import type { BoardMarker, BoardSquareShape } from "../ui/board-geometry";
import type { SerializableBoard, BoardPoint, BoardSquare } from "../game/types";
...
export function buildReplayFrames(
  ...
  for (const [index, historyPoint] of board.m_history.entries()) {
    const owner: ReplayOwner = ownerForReplayMove(firstPlayer, index);
