...
function PieceShape({
  ...
  return (
    <g
      className={`board__piece board__piece--${owner}${fade ? " board__piece--fading" : ""} ${className}`}
      style={fade ? { opacity: fade.opacity } : undefined}
    >
      {fade?.last ? (
        <circle className="board__fade-ring" cx={x} cy={y} r={r * 1.24} />
      ) : null}
      <circle
...
/** Fading pieces: how solid a stone looks, and whether it goes next move. */
export interface PieceFade {
  opacity: number;
  last: boolean;
}

/** A stone that has just washed away, drawn once more as it sinks out. */
export interface WashingStone {
  key: string;
  x: number;
  y: number;
  owner: Owner;
...
export interface BoardDiagramProps {
  ...
  /** Fading pieces, per board index; null or absent draws a solid stone. */
  fade?: readonly (PieceFade | null)[];
  /** Stones that just washed away. */
    ...
    >
      ...
      {washing.map((stone) => (
        <PieceShape
          ...
          y={centre(stone.y)}
          ids={ids}
          className="board__piece--washing"
        />
      ))}
