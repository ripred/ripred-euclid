...
describe("solo rules", () => {
  ...
  it("keeps stones permanent by default and accepts only listed lifetimes", () => {
    const base = {
      W: 8,
      H: 8,
      scoring: "bbox",
    ...
    expect(validatePracticeRules(base).fadeTurns).toBe(0);
    for (const fadeTurns of FADE_TURN_CHOICES) {
      expect(validatePracticeRules({ ...base, fadeTurns }).fadeTurns).toBe(
        fadeTurns,
      );
    }
    for (const fadeTurns of [1, 3, 9, 6.5, "6"]) {
      expect(() => validatePracticeRules({ ...base, fadeTurns })).toThrow(
        RangeError,
      );
    }
  });
