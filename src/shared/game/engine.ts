...
export class Point {
  ...
  static fromJSON(point: SharePoint): Point {
...
export class Square {
  ...
  static fromJSON(square: ShareSquare): Square {
    return new Square(
...
export class Player {
  ...
  static fromJSON(source: SharePlayer): Player {
...
export class Board {
  ...
  static fromJSON(
    ...
    board.playerNames = { ...(source.playerNames ?? {}) };
    board.playerAvatars = { ...(source.playerAvatars ?? {}) };
    board.m_targets = [
      source.m_targets?.[0] ?? null,
      source.m_targets?.[1] ?? null,
    ];
    board.chat = source.chat
      ? {
          seq: source.chat.seq,
          items: source.chat.items.map((item) => ({ ...item })),
        }
      : undefined;
    return board;
  }
